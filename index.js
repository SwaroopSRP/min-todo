...
const closeAddTaskScreenBtn = document.getElementById("close-add-task-screen");
...
function addNewTask(taskText) {
    tasksArr.push({ [tasksArr.length + 1]: taskText });
    taskList.innerHTML += `
        <div class="task-item flex items-center justify-between gap-1.5 border-t-2 border-slate-700 px-3 py-3 hover:bg-gray-800 active:bg-slate-600">
            <p class="task-title text-lg">
                ${taskText}
...
function closeAddTaskScreen() {
    addTaskScreen.classList.remove("flex");
    addTaskScreen.classList.add("hidden");
}

addTaskBtn.addEventListener("click", () => {
    addTaskScreen.classList.remove("hidden");
    addTaskScreen.classList.add("flex");
});

closeAddTaskScreenBtn.addEventListener("click", closeAddTaskScreen());

saveTaskBtn.addEventListener("click", () => {
    closeAddTaskScreen();
    addNewTask(taskTextBox.value);
    taskTextBox.value = "";
});
